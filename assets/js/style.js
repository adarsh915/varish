...
// document.addEventListener("DOMContentLoaded", () => {
//     // Select both navbar and sidebar links
//     const navLinks = document.querySelectorAll(".unique-nav-links li a, .uniqe-nav-side li");
//     const currentPath = window.location.pathname;

//     navLinks.forEach(link => {
//         link.classList.remove("active");

//         // Compare only pathname (ignores domain)
//         if (link.pathname === currentPath) {
//             link.classList.add("active");
//         }
//     });
...
document.addEventListener("DOMContentLoaded", () => {
    // ✅ Target both navbar & sidebar
    const navLinks = document.querySelectorAll(".unique-nav-links li a, .unique-nav-side li a");
    const currentPage = window.location.pathname.split("/").pop(); // e.g. "work.php"
    ...
    // ✅ On click (so it highlights immediately without reload)
    navLinks.forEach(link => {
